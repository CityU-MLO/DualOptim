...
def main(args):
    ...
    if args.only_class >= 0:
        sample_folder_dir += f"/only{args.only_class}"
    if args.exclude_class >= 0:
        sample_folder_dir += f"/ex{args.exclude_class}"
    if rank == 0:
    ...
    for _ in pbar:
        ...
        if args.only_class >= 0:
            y = torch.tensor([args.only_class] * n, device=device)
        else:
            y = torch.randint(0, args.num_classes, (n,), device=device)
            # Optionally exclude a class:
            if args.exclude_class >= 0:
                num_excluded = (y == args.exclude_class).sum().item()
                while num_excluded > 0:
                    y[y == args.exclude_class] = torch.randint(
                        0, args.num_classes, (num_excluded,), device=device
                    )
                    num_excluded = (y == args.exclude_class).sum().item()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--exclude_class", type=int, default=-1)
    parser.add_argument("--only_class", type=int, default=-1)
    ...
    if args.only_class >= 0:
        assert (
            args.exclude_class < 0
        ), "Cannot set both --only_class and --exclude_class at the same time."
    main(args)
