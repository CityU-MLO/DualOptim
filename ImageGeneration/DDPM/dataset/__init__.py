...
def get_dataset(args, config):
    ...
    if config.data.dataset == "CIFAR10":
        dataset = CIFAR10(
            config.data.path, train=True, download=True, transform=tran_transform
        )

    elif config.data.dataset == "STL10":
        # for STL10 use both train and test sets due to its small size
        train_dataset = STL10(
            config.data.path, split="train", download=True, transform=tran_transform
        )
        test_dataset = STL10(
            config.data.path, split="test", download=True, transform=tran_transform
        )
