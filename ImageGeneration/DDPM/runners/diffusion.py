...
class Diffusion(object):
    ...
    def sa_forget(self):
        ...
        for step in range(0, config.training.n_iters):
            ...
            x_remember, c_remember = (
                x_remember.to(self.device),
                c_remember.to(self.device),
            )
    ...
    def generate_mask(self):
        ...
        with torch.no_grad():
            ...
            mask_path = os.path.join(
                "/data/xzhong/ddpm/results/cifar10/mask", str(args.label_to_forget)
            )
    ...
    def sfron_forget(self):
        ...
        optimizer_f = None
        if args.dual:
            config.optim.lr = args.forget_lr
            optimizer_f = get_optimizer(config, model.parameters())
        criteria = torch.nn.MSELoss()
        ...
        for step in range(0, self.config.training.n_iters):
            if args.decay_forget_alpha:
                cur_forget_alpha = cosine_lr_scheduler(
                    args.forget_alpha, step, self.config.training.n_iters
                )
            else:
            ...
            # adaptive gradient ascent
                ...
                ori_forget_loss = -adaptive_loss(
                    ...
                    e,
                    b,
                    lambd=self.config.training.gamma,
                )
            ...
            if (step + 1) % self.config.training.log_freq == 0:
                ...
                logging.info(
                    f"step:{step:04d}, remain L:{ori_remain_loss.item():.4f}, remain a:{args.remain_alpha}, forget L:{ori_forget_loss.item():.4f}, forget a:{cur_forget_alpha:.8f}, time:{end-start:.2f}"
                )
                start = time.time()
    ...
    def generate_fisher(self):
        args, config = self.args, self.config
        logging.info(f"Generating fisher of diffusion to achieve gradient sparsity.")
        ...
        for x, forget_c in D_forget_loader:
            ...
            if train_steps % config.training.log_freq == 0:
                ...
                logging.info(
                    f"Forget (step={train_steps:07d}) Loss: {avg_forget_loss:.4f} Train Steps/Sec: {steps_per_sec:.2f}"
                )
        ...
        for x, remain_c in D_remain_loader:
            ...
            if train_steps % config.training.log_freq == 0:
                ...
                steps_per_sec = log_steps / (end_time - start_time)
                logging.info(
                    f"Remain (step={train_steps:07d}) Loss: {avg_remain_loss:.4f} Train Steps/Sec: {steps_per_sec:.2f}"
                )
                # Reset monitoring variables:
                running_remain_loss = 0
                log_steps = 0
