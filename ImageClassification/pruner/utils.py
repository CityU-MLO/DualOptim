...
def pruning_model(model, px):
    ...
    prune.global_unstructured(
        parameters_to_prune, pruning_method=prune.L1Unstructured, amount=px
    )
...
def pruning_model_structured(model, px):
    ...
    for name, m in model.named_modules():
        if isinstance(m, nn.Conv2d):
            prune.ln_structured(m, name="weight", amount=px, dim=0, n=1)  # l1 loss
...
def pruning_model_random(model, px):
    ...
    parameters_to_prune = tuple(parameters_to_prune)
    prune.global_unstructured(
        parameters_to_prune, pruning_method=prune.RandomUnstructured, amount=px
    )
