...
def main():
    ...
    # load rank config for lora
    if args.rank_config is not None:
        with open(args.rank_config) as f:
    ...
    (
        ...
        test_loader,
        marked_loader,
    ) = utils.setup_model_dataset(args, rank_config)
    model.cuda()
    ...
    if args.resume and checkpoint is not None:
        ...
        if (
            ...
            # load pretrained checkpoint for lora models
            if "lora" in args.arch:
                new_checkpoint = {}
                for k, v in checkpoint.items():
                    if "conv" in k:
                        k_r = k.split(".")
                        k_r = ".".join(k_r[:-1]) + ".conv." + k_r[-1]
                        new_checkpoint[k_r] = checkpoint[k]
                    else:
            ...
            if "lora" in args.arch:
                ...
                utils.mark_bn_as_trainable(model)
