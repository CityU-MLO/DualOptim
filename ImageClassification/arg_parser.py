...
def parse_args():
    ...
    ##################################### Dataset #################################################
    parser.add_argument(
        "--name", type=str, default="vanilla", help="name of the experiment"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--pretrain", action="store_true", help="load pretrained model from checkpoint"
    )
    ...
    parser.add_argument(
        ...
        default=0.01,
        type=float,
        help="initial learning rate for forget set",
    )
    parser.add_argument(
        "--retain_lr",
        default=None,
        type=float,
        help="initial learning rate for retain set",
    )
    parser.add_argument(
        "--unlearn_epochs",
        default=10,
    ...
    parser.add_argument(
        "--rank_config", default=None, type=str, help="the path of rank config"
    )
    parser.add_argument(
        "--eps", default=1 / 255, type=float, help="epsilon for adversarial attack"
    )
    parser.add_argument(
        "--bd_loss", default="noise", type=str, help="loss for backdoor attack"
    )
    parser.add_argument(
        "--bd_lambda", default=1.0, type=float, help="lambda for backdoor attack"
    )
    parser.add_argument(
        "--bd_epoch", default=0, type=int, help="start epoch for backdoor attack"
    )
    parser.add_argument(
        "--bd_fraction",
        default=0.0,
        type=float,
        help="fraction of data for backdoor attack",
    )
    parser.add_argument(
        "--bd_pix",
        default=16,
        type=int,
        help="num of trigger pixels for backdoor attack",
    )
    parser.add_argument("--scrub_alpha", default=0.5, type=float)
    parser.add_argument("--scrub_gamma", default=1.0, type=float)
    parser.add_argument("--sfron_alpha", default=25, type=float)
    parser.add_argument("--sfron_freq", default=5, type=int)
    parser.add_argument("--sfron_iters", default=1500, type=int)
    parser.add_argument("--optim", default="sgd", type=str)
    parser.add_argument("--accum_steps", default=1, type=int)
    parser.add_argument("--awp_lambda", default=0.1, type=float)
    parser.add_argument("--awp_grad_clip", default=1.0, type=float)

    args = parser.parse_args()
    if args.retain_lr is None:
        args.retain_lr = args.unlearn_lr
    return args
