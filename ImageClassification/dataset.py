...
class TinyImageNetDataset(Dataset):
    def __init__(self, image_folder_set, normalize=None, start=0, end=-1):
        self.imgs = []
        self.targets = []
        self.transform = image_folder_set.transform
        self.normalize = normalize
        # print(f"Reading data...")
        for sample in image_folder_set.imgs[start:end]:
            self.targets.append(sample[1])
            img = Image.open(sample[0]).convert("RGB")
            self.imgs.append(transforms.functional.pil_to_tensor(img))
    ...
    def __getitem__(self, idx):
        ...
        if self.transform is not None:
            x = self.transform(x)
        x = div(x.type(FloatTensor), 255.0)
...
class TinyImageNet:
    ...
    def __init__(self, args, normalize=True):
        ...
        if normalize:
            self.norm_layer = transforms.Compose(
                [
        ...
        if "retrain" in args.unlearn:
            self.tr_train.append(transforms.RandAugment(num_ops=2, magnitude=2))
