...
def setup_model_dataset(args, rank=None):
    if args.dataset == "cifar10":
        ...
        if "lora" in args.arch:
            ...
            if args.imagenet_arch:
                model = model_dict[args.arch](
                    num_classes=classes, imagenet=True, rank=rank
                )
            else:
                model = model_dict[args.arch](num_classes=classes, rank=rank)
        else:
            if args.imagenet_arch:
        ...
        if "lora" in args.arch:
            assert rank is not None
            model = model_dict[args.arch](num_classes=classes, imagenet=True, rank=rank)
        else:
            model = model_dict[args.arch](num_classes=classes, imagenet=True)
...
def mark_bn_as_trainable(model: nn.Module) -> None:
...
def freeze_loraB(model: nn.Module) -> None:
    for k, v in model.named_parameters():
        if "lora_B" in k:
            v.requires_grad = False


def unfreeze_loraB(model: nn.Module) -> None:
    for k, v in model.named_parameters():
        if "lora_B" in k:
