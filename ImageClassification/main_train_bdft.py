...
def main():
    ...
    if args.dataset == "imagenet":
        ...
        # teacher_model = setup_model(args)
    else:
        (
            model,
            train_loader,
    ...
    if args.resume:
        ...
        # teacher_model.load_state_dict(checkpoint["teacher_state_dict"], strict=False)
        ...
        print("load pretrained model from checkpoint {}".format(args.checkpoint))
        checkpoint = torch.load(
            args.checkpoint, map_location=torch.device("cuda:" + str(args.gpu))
        )
        model.load_state_dict(checkpoint["state_dict"], strict=False)
        ...
        raise NotImplementedError("Please provide a checkpoint to resume or pretrain")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # acc = bd_erase(train_loader, model, criterion, optimizer, epoch, args)
        acc = bd_train(train_loader, model, criterion, optimizer, epoch, args)
        # acc = bd_train_sam(train_loader, model, criterion, optimizer, epoch, args)
        # acc = bd_ft_anchor(train_loader, model, teacher_model, criterion, optimizer, epoch, args)

        # evaluate on validation set
        tacc = bd_validate(val_loader, model, criterion, args)
        scheduler.step()
